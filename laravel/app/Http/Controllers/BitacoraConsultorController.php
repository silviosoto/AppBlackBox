...
//SELECT `Fecha`, AVG(`Velocidad`) FROM `bitacoras` WHERE YEAR(`Fecha`) =2018 GROUP by MONTH(`Fecha`)
   ...
   // obtiene como parametro id dispositivo la fecha
    function ConsultaPorFecha($id, $idisp ){
        $data_points = array();
        $datos = DB::select("SELECT Month(Fecha) as fecha, AVG(`Velocidad`) as velocidad FROM `bitacoras` WHERE YEAR(`Fecha`) = :fecha and `dispositivos_id` = :idDisp GROUP by Month(Fecha) order by Month(Fecha)",
